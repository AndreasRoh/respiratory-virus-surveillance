...
fludb <- fludb %>%
  mutate(
    ...
    pasient_aldersgruppe = case_when(
      # Opprett aldersgrupper basert på `pasient_alder`
...
if (length(flu_gene_coverage_cols) > 0 && !is.na(run_quality_map_flu$run_col) && !is.na(run_quality_map_flu$date_col)) {
  ...
  for (gene_cov_col in flu_gene_coverage_cols) {
    ...
    p_run_cov_gene <- plot_run_quality_coverage_box_grouped(
      run_quality_gene,
      title_txt = paste("Dekning per run -", gene_label),
...
if (length(flu_gene_coverage_cols) == 0) {
  warning("Fant ingen influensa-genkolonner for dekning; gen-spesifikke kvalitetslysbildene ble ikke laget.")
} else if (is.na(run_quality_map_flu$run_col) || is.na(run_quality_map_flu$date_col)) {
  warning("Influensa-kvalitetslysbildene krever både run- og datokolonner; gen-spesifikke kvalitetslysbildene ble ikke laget.")
}
...
for (current_subtype in ha_mutation_subtype_order) {
  ...
  for (subclade_index in seq_along(subclade_levels)) {
    ...
    subclade_varmekart_data <- subclade_long %>%
    ...
    # Remove overly common/stable mutation profiles to declutter varmekart.
    ...
    subclade_varmekart_plot <- ggplot(
...
if (!is.na(subclade_color_col) && !is.null(virus_col) && "prove_tatt" %in% names(fludb)) {
  flu_dims <- c(
    ...
    "pasient_aldersgruppe" = "Aldersgruppe",
    "pasient_status" = "Pasientstatus",
    "prove_kategori_group" = "Prøvekategorigruppe"
  )
  ...
  for (virus_key in names(virus_map)) {
    ...
    for (dim_col in names(flu_dims)) {
      ...
      p_pair <- build_group_distribution_plots(
        ...
        x_label = dim_label,
        color_label = "Subklade",
        title_prefix = paste0(virus_label, " subklade etter ", dim_label, " - nåværende sesong"),
        palette_base = kvalitativ_comb
      )
