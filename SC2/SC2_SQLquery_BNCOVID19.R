...
disconnect_other_connections <- function(keep_name, env = .GlobalEnv) {
  obj_names <- ls(envir = env, all.names = TRUE)
  conn_names <- obj_names[vapply(
    obj_names,
    function(nm) inherits(get(nm, envir = env, inherits = FALSE), "DBIConnection"),
    logical(1)
  )]
  for (nm in setdiff(conn_names, keep_name)) {
    con_obj <- get(nm, envir = env, inherits = FALSE)
    disconnect_if_valid(con_obj)
    rm(list = nm, envir = env)
  }
  invisible(NULL)
}
disconnect_other_connections("conBNCOVID19")

if (!inherits(conBNCOVID19, "DBIConnection")) {
  stop("Failed to initialize connection object 'conBNCOVID19'.")
}
...
entrytable <- tbl(conBNCOVID19, "ENTRYTABLE") %>% collect()
...
SC2_20_25_raw_merged <- left_join(filtered_entrytable, entryfld, by = "KEY") %>%
  clean_names()

disconnect_if_valid(conBNCOVID19)
rm(conBNCOVID19)
