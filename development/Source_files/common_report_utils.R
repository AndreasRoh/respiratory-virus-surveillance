...
resolve_run_quality_mapping <- function(pathogen, df) {
  ...
  if (pathogen_key == "SC2") {
    ...
    list(
      ...
      cov_col = intersect(c("ngs_coverage_ha", "nc_coverage", "coverage_breadth_artic", "coverage_breadth_swift", "coverage_breadth_eksterne", "coverage_breadth_nano"), names(df))[1],
      qc_col = intersect(c("NC_quality", "nc_quality", "nc_qc_overall_status"), names(df))[1],
...
flu_run_quality_gene_columns <- function(df) {
  intersect(
    c(
      "ngs_coverage_ha",
      "ngs_coverage_na",
...
format_flu_gene_label <- function(gene_col) {
  gene_key <- toupper(gsub("^ngs_coverage_", "", as.character(gene_col)))
  gene_map <- c(
    "HA" = "HA",
    "NA" = "NA",
    "M" = "M",
...
plot_run_quality_coverage_box_grouped <- function(
  run_quality_df,
  title_txt = "Coverage per run",
  color_col = "run_quality_group",
  color_label = "Group",
  color_values = NULL
) {
  if (is.null(run_quality_df) || nrow(run_quality_df) == 0) return(NULL)
  if (!color_col %in% names(run_quality_df)) return(NULL)

  d <- run_quality_df %>%
    dplyr::filter(!is.na(run_quality_coverage_norm)) %>%
    dplyr::mutate(
      color_group = as.character(.data[[color_col]]),
      color_group = ifelse(is.na(color_group) | trimws(color_group) == "", "Ukjent", color_group)
    )
  if (nrow(d) == 0) return(NULL)

  if (is.null(color_values)) {
    color_levels <- unique(d$color_group)
    color_values <- stats::setNames(
      fhi_discrete_palette(length(color_levels), kvalitativ_comb),
      color_levels
    )
  } else {
    missing_levels <- setdiff(unique(d$color_group), names(color_values))
    if (length(missing_levels) > 0) {
      fallback_vals <- fhi_discrete_palette(length(missing_levels), kvalitativ_comb)
      color_values <- c(color_values, stats::setNames(fallback_vals, missing_levels))
    }
  }
